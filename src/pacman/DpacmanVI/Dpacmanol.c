...
chtype c;
struct TipoMsgMonitor msg,monMsg;
...
while (1)
    {
    read(mon_fan[0],&monMsg,sizeof(monMsg));
    if (monMsg.aviso=='n')
        {
        sock=monMsg.x;  // En msg.x viene el numero de socket
	CONTROL=2;
	monMsg.aviso='2';
	}
...
void onlinecontrol(int sock,struct TipoEstado est[],int cantjugadores)
    {
    ...
    /* Conexion a algun socket remoto */
    /* accept devuelve el control si no hay ninguno */
    
    estd.sock=-1;
    estd.sock=accept(sock,(void *)&remtsa,&tams);
    if (estd.sock==-1)
	return;
	
    /* Chequear si hay lugar para el nuevo jugador */
    if (cantjugadores==(C_JUGADOR-1))
	{
	strcpy(msgg,"sinlugar");
	write(estd.sock,&msgg,sizeof(msgg));
	}
    else
	{
	...
	/* Ubicar el primer proceso libre */
	for (i=0;i<C_JUGADOR;i++) 
	    if (est[i].CONTROL==0) id=i;
	
	est[id].sock=estd.sock;
        est[id].CONTROL=2;
	strcpy(est[id].nick,estd.nick);
    
        msg.aviso='n';
	move(7,56);printw("Conectado %d",est[id].sock);
	msg.x=est[id].sock;
...
/* Se deja al socket en modo O_NDELAY para que el accept no sea bloqueante */
...
while (1)
	...
	if ( (accion(MiPanta,msg.x,msg.y,est,msg.id))<0 )
	    ...
	    break;
	onlinecontrol(sock,est,cantjugadores);
	}

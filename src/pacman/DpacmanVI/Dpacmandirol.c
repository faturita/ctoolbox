...
void controltty(char SIMBOLOA,char SIMBOLOB,int CONTROL,int id,int p_Modo,int pip[],int mon_fan[],int sock)
{
...
void demonionline(int sock,int pip[])
    {
    ...
    /* Ejecuta solo un hijo */
    while(1)
    {
    if ( (est.sock=accept(sock,(void *)&remtsa,&tams))<0 )
	{
	printf ("Se ha producido un error al intentar conectarse al cliente...\n");
	//printf ("Error devuelto por el kernel:%s",strerror(errno));
	exit(-1);
    ...
    /* Lectura del nick..... */
    tams=read(est.sock,est.nick,30);
    est.nick[tams]='\0';
    est.CONTROL=2;
    /* Desbloqueo del read */
    fcntl (est.sock,F_SETFL,O_NONBLOCK);
	
    /* Habilitacion al cliente */
    strcpy(msgg,"Habilitado");
    write(est.sock,&msgg,sizeof(msgg));
    /* Inicializacion del estado de ese personaje */
    est.puntos=0;est.vidas=0;
    est.x=POSINIX_F;est.y=POSINIY_F;
    est.SIMBOLO=50+i;
    est.SIMBOLOB=90+i;
    est.p_Modo=0;
    // ojo con este pip[2];
    ...
    /* Envio de la informacion al padre */
    write(pip[1],&est,sizeof(est));
    
    
    }
}

void onlinecontrol(int pipmsg[],int pip[],struct TipoEstado est[])
    {
    int Ppid=getpid();
    int pid;
    struct TipoEstado estd;
    
    estd.sock=-1;
    if ( (read(pip[0],&estd,sizeof(estd)))<0 )
	{
	return;
	}
    ...
    // Atencion con el tamanio del array
    est[C_JUGADOR]=estd;
    est[C_JUGADOR].id=C_JUGADOR;
    move(20,53);printw ("%d",est[C_JUGADOR].id);    
    pid=fork();
    if (Ppid==getppid())
	{
	srand(time(NULL)*(C_JUGADOR+1));
	controltty(est[C_JUGADOR].SIMBOLO,est[C_JUGADOR].SIMBOLOB,2,est[C_JUGADOR].id,est[C_JUGADOR].p_Modo,pipmsg,est[C_JUGADOR].pip,estd.sock);
	close(est[C_JUGADOR].sock);
	endwin();
	exit(0);
	}
    C_JUGADOR++;
...
int pip[2],sock,onlinepip[2];
char msgg[20];
struct TipoMsgMonitor msg;
struct TipoEstado est[C_MAXJUGADOR];
int ciclo;
...
/* Apertura del pipe para conexion con el demonio */
/* Creacion del demonio para establecer las conexiones on line */
pipe(onlinepip);
fcntl(onlinepip[0],F_SETFL,O_NONBLOCK);
demonionline(sock,onlinepip);
...
for (ciclo=0;ciclo<C_JUGADOR;ciclo++)
	...
	controltty(est[ciclo].SIMBOLO,est[ciclo].SIMBOLOB,est[ciclo].CONTROL,ciclo,est[ciclo].p_Modo,pip,est[ciclo].pip,est[ciclo].sock);
...
while (1)
	...
	*/    }
	...
	/* Basandose en la direccion aleatoria que se envio, se calcula la direccion para perseguir 
	al PACMAN */
	if (est[msg.id].CONTROL==0)
	    {
	    msg.direction=pacmandirection(est[msg.id].direction,est[0].x,est[0].y,est[msg.id].x,est[msg.id].y,MiPanta,est[0].p_Modo);
	    }
	...
	onlinecontrol(pip,onlinepip,est);
	}

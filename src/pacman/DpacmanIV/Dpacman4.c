...
int direction;
	...
	{
	if ( (MiPanta[(80*y)+x]==' ') || (MiPanta[(80*y)+x]=='*')  || ((unsigned char)MiPanta[(80*y)+x]==177) )
	    return;
	else
...
int accion(char MiPanta[],int x,int y,struct TipoEstado est[],int id)
	...
	if ((unsigned char)MiPanta[(80*y)+x]==177)
	    {
	    est[id].p_Modo=2;
	    }
...
/*OBJETIVO: dar inteligencia a los perseguidores controlados por la computadora.*/
    ...
    } while ((MiPanta[(80*y)+x]!=' ') && (MiPanta[(80*y)+x]!='*'));

...
/* Cantidad de movimientos iniciales del pacman para salir de la casa */
#define C_MOVINIFAN  10
	...
	if (est[j].p_Modo>0)
		...
		if ( (est[i].x==est[j].x) && (est[i].y==est[j].y) && (i!=j) && (est[i].p_Modo==0) )
			...
			if (est[j].vidas==0)
			    ...
			    /* Resetea el timeout para que se ejecuten rutinas
			    de direccionamiento del fantasma */
...
void inicializar(struct TipoEstado est[],int control)
    {
...
void abrir_puerta(char MiPanta[],int i)
    {
...
void cerrar_puerta(char MiPanta[],int i)
    {
...
int puente_cerrar=0,puente_abrir=0;
int flagpuerta=0,puerta1=0,puerta2=0,puerta3=0,puerta4=0;
...
if ( (argc!=4)||(argv[1][0]<48)||((argv[1][0]-48)>C_JUGADOR) )
    {
    printf ("Copyright (C) Oel Amon Yag Software 1999.\n");
    printf ("\a\nNecesita poner la cantidad de jugadores remotos (valor Maximo:%d)\n",C_JUGADOR-1);
    printf ("Modo de uso>   dpacman jugadores_remotos cantidad_pacman 1|3\n");
    printf ("               1 - Teclado.\n");
    printf ("               3 - Joystick Machuka.\n");   
    exit(-1);
    }
...
if ( (argv[3][0]!=49) && (argv[3][0]-48!=3) )
    {
    printf ("Tipo de control no valido.1 Teclado  -- 2 Joystick.\n\a");
    exit(-1);
    }
...
/* Se le envia el parametro que corresponde al control del jugador en el servidor */
inicializar(est,argv[3][0]-48);
...
/* Muestra presentacion del juego */
presentacion();
...
while (1)
	...
	if (est[msg.id].p_Modo==0)
	    {
	    if ( (est[msg.id].timeout<C_MOVINIFAN) )
		{
		msg.direction=salirfant(est[msg.id].timeout++,msg.id);
		}
	    else if (est[msg.id].CONTROL==0)
		{
	...
	if (!((puerta1)||(puerta2)||(puerta3)||(puerta4)))
	     	{
		cerrar_puerta(MiPanta,puente_cerrar);
		if (!(puente_cerrar>=75))
		    puente_cerrar++;
		puente_abrir=0;
		}
	...
	if ((puerta1)||(puerta2)||(puerta3)||(puerta4))
		...
		if (puente_abrir==0)
		    contador=0;
		abrir_puerta(MiPanta,puente_abrir);
		if (!(puente_abrir>=55))
		    puente_abrir++;
				puente_cerrar=0;
		}
	...
	/* Habilitacion de la direccion que toma el personaje */
	aux_y=est[msg.id].y;
	aux_x=est[msg.id].x;

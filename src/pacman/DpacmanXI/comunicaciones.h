...
printw ("Datos de la red....\n");
printw ("-------------------\n");
printw ("Nombre host completo:%s\n",hp->h_name);
printw ("Alias :\n");
printw ("Familia :%d\n",hp->h_addrtype);
printw ("Tamanio :%d\n",hp->h_length);
printw ("Direccion :%ld\n",hp->h_addr_list[0]);
...
if ((sock=socket(AF_INET,SOCK_STREAM,0))<0)
    ...
    printw ("Verifique la instalacion y vuelvalo a intentar....\n");
...
if (bind(sock,(void *)&hostsa,sizeof (hostsa))<0)
    {
    printw ("\a\nSe produjo un error en el BIND...\n");
    printw ("Verifique que el puerto definido no sea un puerto restrinjido, o que la configuracion de la red es la correcta.\n");
//    printf (strerror(errno));
    exit(-1);
    }
...
//printw ("Datos de HOST....\n");
//printw ("Puerto asignado:%d\n",hostsa.sin_port);    
printw ("Direccion del host:%ld....\n",hostsa.sin_addr);
printw ("Familia de protocolos:%d....\n",hostsa.sin_family);
...
int abrirSocketC(struct sockaddr_in *hosts,char hst[MAXHOSTNAME])
{
...
hp=gethostbyname(hst);
printw ("Lectura del archivo /etc/hosts.....\n\n");

printw ("Informacion del host especificado.  Host remoto ......\n");
printw ("Nombre host completo:%s\n",hp->h_name);
printw ("Familia :%d\n",hp->h_addrtype);
printw ("Tamanio :%d\n",hp->h_length);
printw ("Direccion :%ld\n",hp->h_addr_list[0]);
...
printw ("\nSocket abierto satisfactoriamente.....\n");

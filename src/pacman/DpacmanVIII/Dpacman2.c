...
int accion(char MiPanta[],int x,int y,struct TipoEstado est[],int id)
	...
	if (MiPanta[(80*y)+x]=='*')
	    ...
	    /* Se decrementa el valor porque ahora son varios pacman que comen */
	    MAXPUNTOS--;
...
posicionamiento.
		...
		CONTROL = 4    Obtiene el char mediante el MOUSE
	*/
int Ppid=getpid();
int m,i=0;
int direction=2;
...
while (1)
	...
	case 3:
	    ...
	    switch(inb(JOY+1))
		{
		case 255: c=KEY_LEFT; break;
		case 111: c=KEY_UP;   break;
		case 95:  c=KEY_DOWN; break;
		case 63:  c=KEY_RIGHT;break;
		}
           a=getch();
           if (a=='q')
		{
...
if ( (argv[2][0]<48) || (argv[2][0]-48>cantjugadores+1) )
    {
...
while (1)
	...
	/* Si se produjo una cancelacion de alguno solose le manda a ese */
	if (msg.aviso!='c')
	for (ciclo=0;ciclo<C_JUGADOR;ciclo++)
	    {
	    if (est[ciclo].sock>0)

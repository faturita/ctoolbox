...
void monitor(int pip[])
	...
	/*printw ("Simbolo %c, x %d, y %d:",msg.SIMBOLO,msg.x,msg.y);*/
	}
    }
    

void hijomonitor(int pip[])
    {
    int Ppid=getpid();
    noecho();
    getch();
...
timeout(150);  /* Establece el tiempo de espera del teclado hasta liberar */
...
if (Ppid==getppid())
    {
    controltty('!',0,MiPanta,pip);
    endwin();exit(0);
    }
else
    {
    controltty('@',1,MiPanta,pip);
    endwin();exit(0);
    }

...
void init(weight ***W , neuron ***E) {
    ...
    *E = (neuron **)malloc(sizeof(weight *)*(D+1));
    ...
    for (i=0;i<(D+1);i++) {
     ...
     for (i=0;i<D;i++) {
        (*W)[i]=(weight *)malloc(sizeof(weight)*(Di[i]*Di[i+1]));
        if ((*W)[i]==NULL) {
           printf("No hay suficiente memoria.\n");
           exit(-1);
        }
 ...
 *
 **/
void initLearningPatterns(neuron ***X, neuron ***Y, char *filename) {
   int iSize;
   int p;
   char buffer[MAXSIZE];
 ...
 */
neuron evolve(neuron *X, weight *W,int i, int jMax)
{
	weight aux=0;
   int j= 0;
 ...
 */
neuron* evolveLayeredNN(weight **W, neuron **E) {
 ...
 *
 **/
int getLi(neuron **Li,weight **W, neuron **E,neuron *Y) {
	int i,j,k;
   neuron aux=0;
 ...
 *
 **/
int learn_backprop(weight **W, neuron **E, neuron *Y) {
   weight	dW;
   neuron	**Li;
   int	k,j,i;
   int	bLearn=0;
   ...
   for(k=D;k>0;k--) {
		for (i=0;i<Di[k];i++) {
      	for (j=0;j<Di[k-1];j++) {
            ...
            dW=(weight)((DELTA_WEIGHT) * Li[(k)-1][i] * E[k-1][j] );

        		if (abs(dW) > ACCURACY ) bLearn=1;
 ...
 **/
float getQuadraticError(float **W, float **E, float **X, float **Y, int patternSize) {
	int p,j;
   float fQErr;
...
void logQuadraticError(weight **W, neuron **E, neuron **X, neuron **Y, int patternSize) {
	if (isLogging()) {
 ...
 *
 **/
void learnAll(weight **W, neuron **E, neuron **X,neuron **Y, int patternSize) {
   int bLearn=0;
   int iChance;
   int j;
   int iMUpdate=0;
   ...
   for (j=0;j<patternSize;j++) bLearnVector[j]=0;

   // Log info
   sprintf (logBuffer,"Error cuadratico medio cada %d actualizaciones.\n",patternSize);
   logInfo(logBuffer);
...
int learn(weight **W, neuron **E, neuron *Y) {
	weight dW;
   int j=0;
   int bLearn=0;
